...
struct node *insert(struct node *queue, int A1, int A2){
    struct node *temp = queue, *premin = queue, *item1, *item2;
    while (temp){
        if(temp->next && temp->next->info<premin->next->info){
            premin = temp;
        }
        temp = temp->next;
    }
    ...
    if(!premin->next){
        ...
        item1->next = premin->next;
        if(premin->next && premin->next->next){
            item2->next = premin->next->next;
        }
        if (premin->next && premin->next->next){
            premin->next->next = item2;
        }
        if(premin->next){
            premin->next = item1;
        }
